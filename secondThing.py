# -*- coding: utf-8 -*-
"""
Created on Mon Jul  2 17:03:50 2018
...
for r in range(0,12):
    ...
    matrix = [[0 for x in range(53)] for y in range(574)]
    matrix[0][0] = 'Properties'
    
    for x in range(1,53):
        matrix[0][x]=seq[x-1][r]
        rowno=lettertonum(matrix[0][x])
        for k in range(1,574):
                matrix[k][0]=dataFromCSV[k][2]
                matrix[k][x]=dataFromCSV[k][rowno]
                
    with open("C:/Users/Yekta/Desktop/staj/secondway/"+str(r+1)+".csv", 'w', newline='') as myfile:
